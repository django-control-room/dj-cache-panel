...
test_docker:
	@echo "Starting Docker services with PYTHON_VERSION=$${PYTHON_VERSION:-3.10}
	PYTHON_VERSION=$${PYTHON_VERSION:-3.10} docker compose build
	docker compose up -d
...
test_docker_all:
	...
	@docker compose up -d
	@sleep 3
	@echo "🚀 Starting full test matrix (Python 3.9-3.14, Django 4.2/5.2/6.0)"
	@echo ""
	@bash -c '\
	...
	for py in 3.9 3.10 3.11 3.12 3.13 3.14; do \
	  ...
	  for dj in 4.2 5.2 6.0; do \
	    ...
	    total=$$((total + 1)); combo="py$$py-dj$$dj"; logfile="test-results/$$combo.log"; \
	    printf "  [$$total] Testing $$combo... "; \
	    if docker build -q --build-arg PYTHON_VERSION=$$py --build-arg DJANGO_VERSION=$$dj -f example_project/Dockerfile -t dj-cache-test:$$combo . > /tmp/$$combo.build 2>&1 && \
